...
Infection events, cohort matching and cytopenia tables.

```bash
Rscript analysis/CAR_vs_autoHCT/infection.R private/settings.R
```

Packages used (including conditional analyses): `MatchIt`, `broom`, `cmprsk`, `cobalt`, `dplyr`, `emmeans`, `forcats`, `ggplot2`, `janitor`, `lme4`, `lmerTest`, `purrr`, `readr`, `scales`, `stringr`, `survival`, `survminer`, `tibble`, `tidyr`.

File settings: `PATH_INF`, `PATH_MATCH`, `PATH_CYTO`.
